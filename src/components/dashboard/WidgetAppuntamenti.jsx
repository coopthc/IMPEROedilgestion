...
export default function WidgetAppuntamenti() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const today = all
          .filter((a) => a.data === oggi && a.stato !== "annullato")
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Appuntamenti oggi</h3>
        </div>
        <Link to="/agenda" className="text-[11px] text-primary hover:underline">Agenda →</Link>
      </div>
      {apps.length === 0 ? (
        <p className="text-xs text-muted-foreground italic py-4 text-center">Nessun appuntamento oggi</p>
      ) : (
