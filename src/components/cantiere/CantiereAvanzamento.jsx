...
const STATO_STILI = {
  ...
  in_corso: "bg-blue-500/15 text-blue-400",
  completata: "bg-green-500/15 text-green-400",
  bloccata: "bg-yellow-500/15 text-yellow-400",
  annullata: "bg-red-500/15 text-red-400",
};
...
export default function CantiereAvanzamento({ cantiere, onCantiereUpdate }) {
  ...
  const [nuovoAgg, setNuovoAgg] = useState({ testo: "", tipo: "aggiornamento", visibile_cliente: false });
  ...
  const addAggiornamento = async () => {
    ...
    setNuovoAgg({ testo: "", tipo: "aggiornamento", visibile_cliente: false });
    load();
  };
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-3">
          ...
          <Input
            ...
            onChange={(e) =>
              setNuovaLav((f) => ({ ...f, ore_previste: e.target.value }))
            }
        ...
        {lavorazioni.length === 0 ? (
          <p className="text-xs text-muted-foreground py-3 text-center">
            Nessuna lavorazione. Le lavorazioni create qui compariranno
            automaticamente nella sezione Presenze.
          </p>
        ) : (
          <div className="space-y-2">
            {lavorazioni.map((l) => (
              ...
              >
                ...
                <Select
                  value={l.stato}
                  onValueChange={(v) => updateLavStato(l, v)}
                >
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 text-xs cursor-pointer">
              <Switch
                ...
                onCheckedChange={(v) =>
                  setNuovoAgg((f) => ({ ...f, visibile_cliente: v }))
                }
              />
              Visibile al cliente
            </label>
            <Button size="sm" onClick={addAggiornamento}>
          ...
          <div className="space-y-2">
            {aggiornamenti.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="flex gap-2.5 bg-secondary/30 rounded-lg p-3"
                >
                  <Icon className={`w-4 h-4 flex-shrink-0 mt-0.5 ${tipoInfo.color}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm">{a.testo}</p>
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-destructive/15 text-destructive self-start"
                  >
